<!-- Nav mobile -->
<div class="container">
    <div class="row d-lg-none">
        <div class="col-3">
            <div class="btn-custom-mobile step1 curva-d">
                <a href="#" class="step1">
                    <span class="icon">
                        <svg xmlns="http://www.w3.org/2000/svg" width="9.706" height="16.127" viewBox="0 0 9.706 16.127">
                            <g id="Icon_feather-dollar-sign" data-name="Icon feather-dollar-sign" transform="translate(1 1)">
                                <path id="Trazado_5" data-name="Trazado 5" d="M18,1.5V15.627" transform="translate(-14.147 -1.5)" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>
                                <path id="Trazado_6" data-name="Trazado 6" d="M16.064,7.5H11.248a2.248,2.248,0,1,0,0,4.5h3.211a2.248,2.248,0,1,1,0,4.5H9" transform="translate(-9 -4.931)" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>
                            </g>
                        </svg>
                    </span>
                </a>
            </div>
        </div>
        <div class="col-3">
            <div class="btn-custom-mobile step2">
                <a href="#" class="">
                    <span class="icon">
                        <svg xmlns="http://www.w3.org/2000/svg" width="14.157" height="15.677" viewBox="0 0 14.157 15.677">
                            <g id="Icon_feather-user" data-name="Icon feather-user" transform="translate(1 1)">
                                <path id="Trazado_7" data-name="Trazado 7" d="M18.157,27.059v-1.52A3.039,3.039,0,0,0,15.118,22.5H9.039A3.039,3.039,0,0,0,6,25.539v1.52" transform="translate(-6 -13.382)" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>
                                <path id="Trazado_8" data-name="Trazado 8" d="M18.079,7.539A3.039,3.039,0,1,1,15.039,4.5,3.039,3.039,0,0,1,18.079,7.539Z" transform="translate(-8.961 -4.5)" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>
                            </g>
                        </svg>
                    </span>
                </a>
            </div>
        </div>
        <div class="col-3">
            <div class="btn-custom-mobile step3">
                <a href="#" class="">
                    <span class="icon">
                        <svg xmlns="http://www.w3.org/2000/svg" width="14.236" height="14.236" viewBox="0 0 14.236 14.236">
                            <path d="M5.339,0V5.339H1.78l5.339,5.339,5.339-5.339H8.9V0ZM0,12.457v1.78H14.236v-1.78Z" fill="none" stroke-width="2"/>
                        </svg>
                    </span>
                </a>
            </div>
        </div>
        <div class="col-3">
            <div class="btn-custom-mobile step4">
                <a href="#" class="">
                    <span class="icon asist">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16.661" height="14.695" viewBox="0 0 16.661 14.695">
                            <path id="Icon_awesome-comment-dots" data-name="Icon awesome-comment-dots" d="M7.826,2.25C3.5,2.25,0,5.1,0,8.609a5.644,5.644,0,0,0,1.742,4A7.777,7.777,0,0,1,.067,15.533a.243.243,0,0,0-.046.266.239.239,0,0,0,.223.147,6.991,6.991,0,0,0,4.3-1.571,9.323,9.323,0,0,0,3.283.593c4.323,0,7.826-2.846,7.826-6.359S12.148,2.25,7.826,2.25ZM3.913,9.587a.978.978,0,1,1,.978-.978A.977.977,0,0,1,3.913,9.587Zm3.913,0A.978.978,0,1,1,8.8,8.609.977.977,0,0,1,7.826,9.587Zm3.913,0a.978.978,0,1,1,.978-.978A.977.977,0,0,1,11.739,9.587Z" transform="translate(0.51 -1.75)" fill="none" stroke-width="1"/>
                        </svg>
                    </span>
                </a>
            </div>
        </div>
        <div class="col-12">
            <div class="paso-mobile">
                
            </div>
        </div>
    </div>
</div>
<!-- Fin nav mobile -->   
<div class="container d-none d-lg-block">
    <div class="row">
        <div class="col-md-12">
            <div class="btn-group btn-group-toggle d-flex custom-btn-group" data-toggle="buttons">
                <a href="#" class="btn btn-custom step1" role="button" aria-pressed="true">
                    <span class="icon">
                        <svg xmlns="http://www.w3.org/2000/svg" width="9.706" height="16.127" viewBox="0 0 9.706 16.127">
                            <g id="Icon_feather-dollar-sign" data-name="Icon feather-dollar-sign" transform="translate(1 1)">
                              <path id="Trazado_5" data-name="Trazado 5" d="M18,1.5V15.627" transform="translate(-14.147 -1.5)" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>
                              <path id="Trazado_6" data-name="Trazado 6" d="M16.064,7.5H11.248a2.248,2.248,0,1,0,0,4.5h3.211a2.248,2.248,0,1,1,0,4.5H9" transform="translate(-9 -4.931)" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>
                            </g>
                          </svg>
                    </span>Pago del manual
                    <span class="icon-arrow">
                        <svg  xmlns="http://www.w3.org/2000/svg" width="10.305" height="17.61" viewBox="0 0 10.305 17.61">
                            <path id="Icon_feather-chevron-down" data-name="Icon feather-chevron-down" d="M0,0,6.684,6.684,13.368,0" transform="translate(2.121 15.489) rotate(-90)" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="3"/>
                        </svg>
                    </span>
                </a>
                <a href="#" class="btn btn-custom step2" role="button" aria-pressed="true">
                    <span class="icon">
                        <svg xmlns="http://www.w3.org/2000/svg" width="14.157" height="15.677" viewBox="0 0 14.157 15.677">
                            <g id="Icon_feather-user" data-name="Icon feather-user" transform="translate(1 1)">
                                <path id="Trazado_7" data-name="Trazado 7" d="M18.157,27.059v-1.52A3.039,3.039,0,0,0,15.118,22.5H9.039A3.039,3.039,0,0,0,6,25.539v1.52" transform="translate(-6 -13.382)" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>
                                <path id="Trazado_8" data-name="Trazado 8" d="M18.079,7.539A3.039,3.039,0,1,1,15.039,4.5,3.039,3.039,0,0,1,18.079,7.539Z" transform="translate(-8.961 -4.5)" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>
                            </g>
                        </svg>
                    </span>Perfil replicado
                    <span class="icon-arrow">
                        <svg  xmlns="http://www.w3.org/2000/svg" width="10.305" height="17.61" viewBox="0 0 10.305 17.61">
                            <path id="Icon_feather-chevron-down" data-name="Icon feather-chevron-down" d="M0,0,6.684,6.684,13.368,0" transform="translate(2.121 15.489) rotate(-90)" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="3"/>
                        </svg>
                    </span>
                </a>
                <a href="#" class="btn btn-custom step3" role="button" aria-pressed="true">
                    <span class="icon">
                        <svg xmlns="http://www.w3.org/2000/svg" width="14.236" height="14.236" viewBox="0 0 14.236 14.236">
                            <path d="M5.339,0V5.339H1.78l5.339,5.339,5.339-5.339H8.9V0ZM0,12.457v1.78H14.236v-1.78Z" fill="none"/>
                        </svg>
                    </span>Descarga de manual
                    <span class="icon-arrow">
                        <svg  xmlns="http://www.w3.org/2000/svg" width="10.305" height="17.61" viewBox="0 0 10.305 17.61">
                            <path id="Icon_feather-chevron-down" data-name="Icon feather-chevron-down" d="M0,0,6.684,6.684,13.368,0" transform="translate(2.121 15.489) rotate(-90)" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="3"/>
                        </svg>
                    </span>
                </a>
                <a href="#" class="btn btn-custom step4" role="button" aria-pressed="true">
                    <span class="icon">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16.661" height="14.695" viewBox="0 0 16.661 14.695">
                            <path id="Icon_awesome-comment-dots" data-name="Icon awesome-comment-dots" d="M7.826,2.25C3.5,2.25,0,5.1,0,8.609a5.644,5.644,0,0,0,1.742,4A7.777,7.777,0,0,1,.067,15.533a.243.243,0,0,0-.046.266.239.239,0,0,0,.223.147,6.991,6.991,0,0,0,4.3-1.571,9.323,9.323,0,0,0,3.283.593c4.323,0,7.826-2.846,7.826-6.359S12.148,2.25,7.826,2.25ZM3.913,9.587a.978.978,0,1,1,.978-.978A.977.977,0,0,1,3.913,9.587Zm3.913,0A.978.978,0,1,1,8.8,8.609.977.977,0,0,1,7.826,9.587Zm3.913,0a.978.978,0,1,1,.978-.978A.977.977,0,0,1,11.739,9.587Z" transform="translate(0.51 -1.75)" fill="none" stroke-width="1"/>
                        </svg>
                    </span>Sesiones de asistencia
                    <span class="icon-arrow">
                        <svg  xmlns="http://www.w3.org/2000/svg" width="10.305" height="17.61" viewBox="0 0 10.305 17.61">
                            <path id="Icon_feather-chevron-down" data-name="Icon feather-chevron-down" d="M0,0,6.684,6.684,13.368,0" transform="translate(2.121 15.489) rotate(-90)" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="3"/>
                        </svg>
                    </span>
                </a>
            </div>
        </div>
    </div>
</div>